...
I ran it first in our main VPCs, then designed it into the cell architecture ([environments you can create and destroy with one command](/case-studies/environments-you-can-create-and-destroy-with-one-command/)) as a per-environment toggle — on ARM instances, naturally ([turning container images from a liability into a supply chain](/case-studies/turning-container-images-from-a-liability-into-a-supply-chain/) made that free). A later overhaul pre-allocates all the public IPs with the VPC — the NAT instances' and the standby gateways' — so flipping the toggle never changes the addresses the outside world has allowlisted.
